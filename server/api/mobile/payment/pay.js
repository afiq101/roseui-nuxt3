...
import { CheckTransactionExist, UpdateKempenBillCode, UpdatePaymentBillcode, UpdateWakafAmPaymentBillcode } from "~~/server/model/mobile/payment/billpayment";
        ...
        else if(userSecretKey === "" || userSecretKey === null) 
        {
            ...
            if(verify_user.status === false) {
                ...
                if(check_kalau_transaction_wujud.status) {
                    ...
                    let billType        = bill.Billpayment_Type; // Wakaf || Sumbangan || Wakaf Am
                    ...
                    if(create_bill_toyyibpay.status === true && create_bill_toyyibpay.billcode) {
                        ...
                        // UPDATE WAKAF AM TRANS BILLCODE
                        if(billType === "Wakaf Am") {
                            let update_wakafam_payment_bill = await UpdateWakafAmPaymentBillcode(billInvoiceNo, create_bill_toyyibpay.billcode);
                            console.log("Log Update Wakaf Am Payment BillCode : ", update_wakafam_payment_bill);
                        }

                        if(billType === "Kempen") {
                            let update_kempen_bill_code = await UpdateKempenBillCode(billInvoiceNo, create_bill_toyyibpay.billcode);
                            console.log("Log Function Update Kempen BillCode : ", update_kempen_bill_code);
                        }
